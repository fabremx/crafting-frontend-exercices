...
const STARTING_BET_IDENTIFIER = 'wcf-starting-bet';
const STARTING_BET_CONTENT_IDENTIFIER = '.starting-bet';
const BETS_SUMMARY_IDENTIFIER = 'wcf-bets-summary';
const BETS_SUMMARY_CONTENT_IDENTIFIER = '.bets-summary';
...
describe('Bets Page', () => {
    ...
    it('should display correct bets information when user select 2 bets (odds 1.24 and 2.50) with 100 € as starting bet', async () => {
        ...
        const summaryContent = await page.$eval(`pierce/${BETS_SUMMARY_CONTENT_IDENTIFIER}`, (e) => e.textContent);
        expect(summaryContent).toContain('Nombre de paris joués: 2');
        expect(summaryContent).toContain('Potentiel gain: 151.8');
    });
});
...
function selectStartingBetInput(page: puppeteer.Page) {
    return page.$(`pierce/${STARTING_BET_CONTENT_IDENTIFIER} input`);
}